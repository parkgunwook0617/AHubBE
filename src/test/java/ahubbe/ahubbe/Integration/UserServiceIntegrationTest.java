...
public class UserServiceIntegrationTest {
    ...
    void specificTitleDataQuery() {
        Assertions.assertThat(userService.importAllAnimationInformationByTitle("판").size()).isEqualTo(2);
        Assertions.assertThat(userService.importAllAnimationInformationByTitle("s").size() ).isEqualTo(8);
        Assertions.assertThat(userService.importAllAnimationInformationByTitle("가").size()).isEqualTo(15);
    }

    @Test
    @DisplayName("장르 조회가 잘 되는지 확인")
    void specificGenreQuery() {
        Assertions.assertThat(userService.importAnimationInformationByGenre("판타지").size()).isEqualTo(16);
        Assertions.assertThat(userService.importAnimationInformationByGenre("하렘").size()).isEqualTo(4);
        Assertions.assertThat(userService.importAnimationInformationByGenre("일상").size()).isEqualTo(14);
    }

    @Test
    @DisplayName("연도 조회가 잘 되는지 확인")
    void specificReleaseYearQuery() {
        Assertions.assertThat(userService.importAnimationInformationByReleaseYear("2025").size()).isEqualTo(81);
        Assertions.assertThat(userService.importAnimationInformationByReleaseYear("2024").size()).isEqualTo(0);
    }

    @Test
    @DisplayName("분기 조회가 잘 되는지 확인")
    void specificReleaseQuarterQuery() {
        Assertions.assertThat(userService.importAnimationInformationByReleaseQuarter("2").size()).isEqualTo(81);
        Assertions.assertThat(userService.importAnimationInformationByReleaseQuarter("1").size()).isEqualTo(0);
        Assertions.assertThat(userService.importAnimationInformationByReleaseQuarter("3").size()).isEqualTo(0);
        Assertions.assertThat(userService.importAnimationInformationByReleaseQuarter("4").size()).isEqualTo(0);
    }

    @Test
    @DisplayName("단일 제목 조회가 잘 되는지 확인")
    void singleSpecificTitleQuery() {
        AnimationInformation targetData = userService.importSingleAnimationInformationByTitle("Summer Pockets/애니메이션")
